...
const CandidateStackDetailsModal = ({ open, onClose, data, onStatusChange }) => {
  ...
  const calculateDetailedBreakdown = (ctc) => {
    ...
    // Monthly Gross = Basic + HRA + Conv + Edu + Special
    // Monthly CTC = Monthly Gross + Employer PF + Bonus
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-[1000] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-lg w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
  ...
  {/* Row 2: JOB TITLE (Image lo unattu add chesa) */}
          ...
          <div className="border border-gray-300 rounded shadow-sm">
            <table className="w-full border-collapse">
              ...
              <tbody>
                ...
                {/* <TableHeader title="V. Fixed Cost to Company" colorClass="bg-blue-900" /> */}
    ...
    {[
      ...
      <div key={idx} className="flex items-start gap-2 text-[11px] text-gray-700 leading-tight">
        ...
        <p>{point}</p>
          ...
          {/* Remarks */}
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-1">Approval Remarks</label>
            <textarea 
        ...
        <div className="p-4 bg-gray-50 border-t flex justify-end gap-3">
          <button onClick={onClose} className="px-6 py-2 border border-gray-300 rounded font-bold text-gray-600 hover:bg-gray-100">CANCEL</button>
          <button onClick={() => onStatusChange({...data, status: 'rejected', remarks})} className="px-6 py-2 bg-red-600 text-white rounded font-bold hover:bg-red-700">REJECT</button>
